...
void main() {
  ...
  group('LocationHandler', () {
    ...
    test('constructor registers the Pigeon callback handler', () async {
      ...
      await messenger.handlePlatformMessage(
        'dev.flutter.pigeon.amap_kit_location_android.LocationFlutterApi.onLocation',
  ...
  test('silently ignores iOS-only options when calling the host channel', () async {
    AmapLocationAndroid.registerWith();
    ...
    messenger.setMockMessageHandler(
      'dev.flutter.pigeon.amap_kit_location_android.LocationHostApi.createClient',
    ...
    messenger.setMockMessageHandler(
      'dev.flutter.pigeon.amap_kit_location_android.LocationHostApi.setLocationOption',
      (ByteData? message) async {
        final List<Object?>? decoded =
            LocationHostApi.pigeonChannelCodec.decodeMessage(message)
                as List<Object?>?;
    ...
    // 不抛不支持错误，且发给宿主的选项不含 iOS 专属配置。
    expect(sentOptions, hasLength(2));
    expect(
      sentOptions.every((NativeLocationOptions o) => o.android == null),
      isTrue,
    );
  });
}
